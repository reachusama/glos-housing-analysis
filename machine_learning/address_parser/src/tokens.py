...
# synonyms.csv → dict `synonym_LUT` mapping from -> to
synonyms_path = os.path.join(data_dir, 'synonyms.csv')
df_synonyms = pd.read_csv(synonyms_path, usecols=['from', 'to'], dtype=str, encoding='utf-8-sig')
df_synonyms = df_synonyms.dropna(subset=['from', 'to'])
synonym_LUT = dict(
