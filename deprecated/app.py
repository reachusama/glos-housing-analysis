...
def kpi_card(
    label: str, value, delta: Optional[str] = None, help: Optional[str] = None
):
    c = st.container()
    with c:
...
st.markdown("By [Usama Shahid - LinkedIn](https://www.linkedin.com/in/reach-usama/) \n")
...
if nav == "Trends & Seasonality":
    ...
    m = base.copy()
    pivot = (
        m.groupby(["Year", "Month"])[price_col]
        .median()
        .reset_index()
...
if nav == "Distributions":
    ...
    fig = px.histogram(
        x=s, nbins=bins, labels={"x": "log(Price)" if log_scale else "Price"}
    )
    st.plotly_chart(fig, use_container_width=True)
...
if nav == "Hedonic model":
    ...
    model_df = base.dropna(
        subset=["Price", "Property Type", "Old/New", "Duration", "Year", "Month"]
    ).copy()
